...
async function seed() {
  try {
    await mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/nexaforcedb');
    ...
    const existing = await User.findOne({ email: 'admin@nexaforce.com' });
    if (!existing) {
      await User.create({ name: 'Admin', email: 'admin@nexaforce.com', password: 'Admin@123', role: 'admin' });
      console.log('Admin user created: admin@nexaforce.com / Admin@123');
    } else {
